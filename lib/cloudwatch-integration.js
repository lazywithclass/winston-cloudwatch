  ...
  function safeUpload(cb) {
    lib.getToken(aws, groupName, streamName, retentionInDays, options, function(err, token) {
      ...
      aws.putLogEvents(payload, function(err, data) {
        ...
        if (err) {
          // DataAlreadyAcceptedException means that we either already successfully
          // sent this message or the payload (except the timestamp) and sequence 
          // token is the same
          if (options.ignoreDataAlreadyAcceptedException && err.code === "DataAlreadyAcceptedException") {
            debug(err.code, ", ignoring, cleared token");
            lib._postingEvents[streamName] = false;
            clearToken(); // we need to clear token to avoid the same error again
            cb();
          }
