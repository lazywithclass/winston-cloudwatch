    ...
    lib.getToken(aws, groupName, streamName, retentionInDays, function(err, token) {
      ...
      aws.putLogEvents(payload, function(err, data) {
        if (err) {
          ...
          // also do the same if ResourceNotFound as that will result in  the last token for the group being set to null
          // and ensureGroup + getStream being called which results in the resource being created if not exists
          ...
          // save the next sequence token if exists
          if(data && data.nextSequenceToken) {
              lib.getToken.lastNextToken[previousKeyMapKey(groupName, streamName)] = data.nextSequenceToken;
          }
...
lib.submitWithAnotherToken = function(aws, groupName, streamName, payload, retentionInDays, cb) {
    lib.getToken.lastNextToken[previousKeyMapKey(groupName, streamName)] = null;
    lib.getToken(aws, groupName, streamName, retentionInDays, function(err, token) {
...
lib.getToken = function(aws, groupName, streamName, retentionInDays, cb) {
  var existingNextToken = lib.getToken.lastNextToken[previousKeyMapKey(groupName, streamName)];
  if(existingNextToken !== null && existingNextToken !== undefined) {
